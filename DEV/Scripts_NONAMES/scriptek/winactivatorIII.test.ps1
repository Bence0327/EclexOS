...
# Remove temporary MAS script files after execution
$FilePaths = @("$env:SystemRoot\Temp\MAS*.cmd", "$env:USERPROFILE\AppData\Local\Temp\MAS*.cmd")
foreach ($FilePath in $FilePaths) { Get-Item $FilePath | Remove-Item }

# Trigger Windows activation
Write-Host "Activating Windows..."
Start-Process -FilePath "slmgr.vbs" -ArgumentList "/ato" -Wait

# Trigger Office activation
Write-Host "Activating Office..."
$officeActivationScript = "$env:ProgramFiles\Microsoft Office\Office16\ospp.vbs"
if (Test-Path $officeActivationScript) {
    Start-Process -FilePath $officeActivationScript -ArgumentList "/act" -Wait
} else {
    Write-Warning "Office activation script not found at $officeActivationScript"
}
